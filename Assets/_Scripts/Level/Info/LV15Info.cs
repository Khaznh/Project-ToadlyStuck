...
public class LV15Info : LVInfo
{
    ...
    {
        PlayerController.Instance.playerMovement.isBlockJump = false;
        PlayerController.Instance.playerMovement.OnPlayerJump -= RestrictJump;
    }
    ...
    private void RestrictJump()
    {
        ...
        if (currentJumpCount >= maxJumpCount)
        {
            PlayerController.Instance.playerMovement.isBlockJump = true;
        }
    }
    ...
    public void OnPlayerNextLevel(Collider2D collision, Activer activer)
    {
        ...
        LevelUnlockManager.Instance.UnlockNextLevel(16);
